...
v7: applied_rate on applications; town + postal_code on requests;
    actioned_by lock on matches.
"""
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                CREATE TABLE IF NOT EXISTS applications (
                    ...
                    applied_rate INTEGER DEFAULT 0,
                    actioned_by  BIGINT DEFAULT NULL,
                    created_at   TIMESTAMPTZ DEFAULT NOW(),
            ...
            # ── SAFE MIGRATIONS (run every boot, harmless if column exists) ────
            for sql in [
                "ALTER TABLE applications ADD COLUMN IF NOT EXISTS applied_rate INTEGER DEFAULT 0",
                "ALTER TABLE applications ADD COLUMN IF NOT EXISTS actioned_by  BIGINT  DEFAULT NULL",
                "ALTER TABLE requests     ADD COLUMN IF NOT EXISTS town         TEXT    DEFAULT ''",
                "ALTER TABLE requests     ADD COLUMN IF NOT EXISTS postal_code  TEXT    DEFAULT ''",
                "ALTER TABLE matches      ADD COLUMN IF NOT EXISTS actioned_by  BIGINT  DEFAULT NULL",
            ]:
                cur.execute(sql)
            conn.commit()
