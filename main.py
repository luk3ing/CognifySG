...
async def show_req_card(q, ctx):
    ...
    ref    = field('Ref',    f"#{r['id']}")
    budget = field('Budget', f"${r['budget']}/hr")
    await q.edit_message_text(
...
async def apply_request(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if tutor and req:
        ...
        await notify_all_admins(
            ...
            "{div}\n👨‍🏫  *TUTOR*\n{tname}\n{tphone}\n{thandle}\n{tsubjs}\n{tlvls}\n{trate}\n\n"
            "{div}\n👨‍👩‍👧  *PARENT*\n{pname}\n{pphone}\n{phandle}\n\n{div}\n"
            "✅  _Contact both parties on WhatsApp to confirm the match._".format(
                header=header('🎯', 'New Application — Action Required'),
                div=DIV2,
                ref=field('Ref',     f"#{req['id']}"),
                subj=field('Subject', req['subject']),
                lvl=field('Level',   req['level']),
                area=field('Area',   req['areas']),
                budget=field('Budget', f"${req['budget']}/hr"),
                tname=field('Name',     tutor['name']),
                tphone=field('WhatsApp', tutor['phone']),
                thandle=field('Telegram', f"@{tutor['username']}" if tutor['username'] else "none"),
                tsubjs=field('Subjects', tutor['subjects']),
                tlvls=field('Levels',   tutor['levels']),
                trate=field('Rate',     f"${tutor['rate']}/hr"),
                pname=field('Name',     req['name']),
                pphone=field('WhatsApp', req['phone']),
                phandle=field('Telegram', f"@{req['username']}" if req['username'] else "none"),
            )
        )
    conn.close()
    await q.edit_message_text(
