...
Changes from v6:
- Parent info reuse on repeat requests (skip name/phone if already on file)
- Education level shortforms (Pri 1-3, Sec 1-2, JC 1-2, etc.)
- Targeted tutor DM notifications on new request (subject + area match)
- Role lock: tutor stays tutor, parent stays parent — no switching
"""
...
# Change 2: Short education level labels
ALL_LEVELS = ["Pri 1-3", "Pri 4-6", "Sec 1-2", "Sec 3-4", "Sec 5", "JC 1-2", "IB/IP", "Poly/ITE"]
    ...
    if not admins:
        if SUPER_ADMIN_ID:
            try:
                ...
                logger.error("Failed to notify super admin: %s", e)
        return
    for aid in admins:
        try:
...
def get_user_role(uid):
    """Returns 'tutor', 'parent', or None."""
    row = db.execute("SELECT role FROM user_roles WHERE user_id=%s", (uid,), fetch="one")
    return row["role"] if row else None

def set_user_role(uid, role):
    db.execute(
        "INSERT INTO user_roles (user_id, role) VALUES (%s, %s) ON CONFLICT (user_id) DO NOTHING",
        (uid, role)
    )
...
async def error_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    logger.error("Unhandled exception for user %s: %s", uid, err, exc_info=True)
    try:
        db.execute(
...
(TERMS,
 ...
 P_CONFIRM, P_NAME, P_PHONE, P_SUBJECT, P_LEVEL, P_AREA, P_BUDGET,
 EDIT_TUTOR_MENU, EDIT_NAME, EDIT_PHONE, EDIT_SUBJECTS, EDIT_LEVELS, EDIT_AREAS, EDIT_RATE) = range(23)
...
async def captcha_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if chosen == ctx.user_data.get("captcha_ans"):
        # Change 4: auto-route returning users to their existing role dashboard
        ...
        if role == "tutor":
            ...
            await q.edit_message_text(
                hdr("⏳", "Approval Pending") + "\n\nYour tutor profile is under review. You'll be notified once approved.",
                parse_mode="Markdown"
            )
            return ConversationHandler.END
        elif role == "parent":
...
async def role_select(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Change 4: enforce role lock
    existing_role = get_user_role(uid)

    if q.data == "role_tutor":
        if existing_role == "parent":
            await q.edit_message_text(
                hdr("🔒", "Role Locked") + "\n\n"
                "You are registered as a *parent* and cannot switch to tutor.\n\n"
                "_Use /start to return to your parent dashboard._",
                parse_mode="Markdown"
            )
            return ConversationHandler.END

        # Lock role immediately (even before registration completes)
        set_user_role(uid, "tutor")
        ...
        if existing_role == "tutor":
            await q.edit_message_text(
                hdr("🔒", "Role Locked") + "\n\n"
                "You are registered as a *tutor* and cannot switch to parent.\n\n"
                "_Use /start to return to your tutor dashboard._",
                parse_mode="Markdown"
            )
            return ConversationHandler.END

        # Lock as parent
        set_user_role(uid, "parent")
        return await parent_menu(update, ctx)
...
async def t_phone(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if db.execute("SELECT 1 FROM tutors WHERE phone=%s", (txt,), fetch="one"):
        await update.message.reply_text(
            "⚠️ This phone number is already registered.", parse_mode="Markdown"
        )
        return T_PHONE
...
async def t_rate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Change 4: confirm role lock in DB (already set at role_select, but ensure consistency)
    set_user_role(u.id, "tutor")
...
async def applied_postings(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for a in apps:
        ...
        lines.append(
            ...
            "   Score: " + str(a["match_score"]) + "/100 | Status: " + status_icon + "\n"
...
async def edit_subjects(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    current = [s.strip() for s in tutor["subjects"].split(",")] if tutor["subjects"] else []
...
async def edit_levels(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not tutor:
        return await edit_profile_menu(update, ctx)
    current = [l.strip() for l in tutor["levels"].split(",")] if tutor["levels"] else []
...
async def edit_areas(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not tutor:
        return await edit_profile_menu(update, ctx)
    current = [a.strip() for a in tutor["areas"].split(",")] if tutor["areas"] else []
...
async def update_phone(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id
    existing = db.execute(
        "SELECT user_id FROM tutors WHERE phone=%s AND user_id!=%s", (txt, uid), fetch="one"
    )
    if existing:
        await update.message.reply_text(
...
async def parent_menu(update, ctx):
    ...
    saved = db.execute(
        "SELECT name FROM requests WHERE parent_id=%s ORDER BY created_at DESC LIMIT 1",
        (uid,), fetch="one"
    )
    parent_name = saved["name"] if saved else "there"

    req_count_row = db.execute(
        "SELECT COUNT(*) as n FROM requests WHERE parent_id=%s", (uid,), fetch="one"
    )
    req_count = req_count_row["n"] if req_count_row else 0
...
async def post_req_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    Change 1: if parent has saved name+phone, show confirmation step and skip
    directly to subject selection. Otherwise start the full registration.
    """
    q = update.callback_query
    await q.answer()
    uid = q.from_user.id
    ...
    saved = db.execute(
        "SELECT name, phone FROM requests WHERE parent_id=%s ORDER BY created_at DESC LIMIT 1",
        (uid,), fetch="one"
    )

    if saved:
        # Store saved info and jump to confirmation
        ctx.user_data["p_name"]  = saved["name"]
        ctx.user_data["p_phone"] = saved["phone"]
        ctx.user_data["p_saved"] = True

        kb = [
            [InlineKeyboardButton("✅ Yes, use saved details", callback_data="p_use_saved")],
            [InlineKeyboardButton("✏️ Update my details",      callback_data="p_update_details")],
        ]
        await q.edit_message_text(
            hdr("📝", "New Tutor Request") + "\n\n"
            "Using your saved contact details:\n\n" +
            fld("Name", saved["name"]) + "\n" +
            fld("WhatsApp", saved["phone"]) + "\n\n" +
        ...
        # First-time parent — full flow
        ctx.user_data["p_saved"] = False
...
async def p_confirm_saved(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Use saved details, jump straight to subject selection."""
    q = update.callback_query
    await q.answer()
    ctx.user_data["p_subject"] = []
    await q.edit_message_text(
        hdr("📚", "Subject Required") + "\n\n_Step 1 of 3_ — Select subject(s):",
        reply_markup=ms_kb(ALL_SUBJECTS, [], "psubj"),
        parse_mode="Markdown"
    )
    return P_SUBJECT

async def p_update_details(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Parent chose to update their details — restart from name."""
    q = update.callback_query
    await q.answer()
    ctx.user_data["p_saved"] = False
    await q.edit_message_text(
        hdr("📝", "Update Contact Details") + "\n\n_Step 1 of 5_ — Enter your *full name:*",
        parse_mode="Markdown"
    )
    return P_NAME
...
async def p_phone(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["p_subject"] = []
    await update.message.reply_text(
        hdr("📚", "Subject Required") + "\n\n_Step 3 of 5_ — Select subject(s):",
        reply_markup=ms_kb(ALL_SUBJECTS, [], "psubj"),
        parse_mode="Markdown"
    )
    return P_SUBJECT
...
async def p_subject(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if val == "DONE":
        ...
        step = "Step 2 of 3" if ctx.user_data.get("p_saved") else "Step 4 of 5"
        await q.edit_message_text(
            hdr("🎓", "Academic Level") + "\n\n_" + step + "_\n\n"
...
async def p_level(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if val == "DONE":
        ...
        step = "Step 2 of 3 (cont.)" if ctx.user_data.get("p_saved") else "Step 4 of 5 (cont.)"
        await q.edit_message_text(
            hdr("📍", "Location") + "\n\n_" + step + "_\n\n"
...
async def p_area(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if val == "DONE":
        if not ctx.user_data.get("p_area"):
            await q.answer("Pick at least one area!", show_alert=True)
            return P_AREA
        step = "Step 3 of 3" if ctx.user_data.get("p_saved") else "Step 5 of 5"
        await q.edit_message_text(
            hdr("💰", "Budget") + "\n\n_" + step + "_\n\n"
...
async def p_budget(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not valid_rate(txt):
        await update.message.reply_text(
            "⚠️ *Invalid budget.* Enter a positive number.\n_Example: `35`_", parse_mode="Markdown"
        )
        return P_BUDGET
    ...
    try:
        ...
        await update.message.reply_text(
            hdr("✅", "Request Live") + "\n\n"
            "Your request is now *live* and visible to matching tutors.\n\n" +
        ...
        # Background: sheets + admin notify + tutor DM notifications
        p_name_val   = ctx.user_data["p_name"]
        p_phone_val  = ctx.user_data["p_phone"]
        p_subject_val = ", ".join(ctx.user_data["p_subject"])
        p_level_val   = ", ".join(ctx.user_data["p_level"])
        p_area_val    = ", ".join(ctx.user_data["p_area"])

        async def background_tasks():
            try:
                await log_to_sheets_async(
                    sheets.log_request, req_id, p_name_val, p_phone_val,
                    u.username or "", p_subject_val, p_level_val, p_area_val, budget
                )
            ...
            try:
                ...
                msg_txt = (
                    ...
                    fld("Name",    p_name_val) + "\n" +
                    fld("WhatsApp", p_phone_val) + "\n" +
                    fld("Telegram", handle) + "\n" +
                    fld("Subject", p_subject_val) + "\n" +
                    fld("Level",   p_level_val) + "\n" +
                    fld("Area",    p_area_val) + "\n" +
                    fld("Budget",  rate_str(budget)) + "\n\n" +
                    DIV2 + "\n_Request #" + str(req_id) + " — auto-approved and live._"
                )
                await notify_admins(update.get_bot(), msg_txt)
            except Exception as e:
                logger.error("Admin notification failed: %s", e)

            # Change 3: notify matching tutors by DM
            try:
                await notify_matching_tutors(
                    update.get_bot(), req_id, p_subject_val, p_level_val, p_area_val, budget
                )
            except Exception as e:
                logger.error("Tutor notification failed: %s", e)
...
# ── CHANGE 3: NOTIFY MATCHING TUTORS ──────────────────────────────────────────
async def notify_matching_tutors(bot, req_id, req_subject, req_level, req_areas, req_budget):
    """
    DM all approved, available tutors whose subjects AND areas overlap the new request.
    Each tutor gets a direct 'Apply Now' button. No channel needed.
    """
    tutors = db.execute(
        "SELECT user_id, name, subjects, levels, areas, rate FROM tutors WHERE approved=1 AND available=1",
        fetch="all"
    )
    if not tutors:
        return

    r_subjects = [s.strip().lower() for s in req_subject.split(",")]
    r_areas    = [a.strip().lower() for a in req_areas.split(",")]
    ...
    for tutor in tutors:
        ...
        subject_match = any(s in t_subjects for s in r_subjects)
        area_match    = any(a in t_areas for a in r_areas) or "online" in t_areas

        if not (subject_match and area_match):
            continue

        kb = [[InlineKeyboardButton("✅ Apply Now", callback_data="apply_" + str(req_id))]]
        try:
            await bot.send_message(
                tutor["user_id"],
                hdr("🔔", "New Matching Request!") + "\n\n" +
                fld("Subject", req_subject) + "\n" +
                fld("Level",   req_level) + "\n" +
                fld("Area",    req_areas) + "\n" +
                fld("Budget",  rate_str(req_budget)) + "\n\n" +
                DIV2 + "\n_Tap below to apply. Contact details revealed on match._",
                reply_markup=InlineKeyboardMarkup(kb),
                parse_mode="Markdown"
            )
            notified += 1
        except Exception as e:
            logger.warning("Could not notify tutor %s: %s", tutor["user_id"], e)

    logger.info("Notified %s tutor(s) for request #%s", notified, req_id)
...
async def my_reqs(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not reqs:
        ...
        await q.edit_message_text(
            hdr("📋", "My Requests") + "\n\nNo requests posted yet. Tap below to create one:",
            reply_markup=InlineKeyboardMarkup(kb),
            parse_mode="Markdown"
        )
        return
...
async def browse_reqs(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not reqs:
        await q.edit_message_text(
            ...
            "_You'll get a notification when a matching request is posted._",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back_t")]]),
            parse_mode="Markdown"
        )
        return
...
async def apply_req(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Guard: must be an approved tutor
    tutor = db.execute("SELECT * FROM tutors WHERE user_id=%s AND approved=1", (tutor_id,), fetch="one")
    if not tutor:
        await q.answer("⚠️ Only approved tutors can apply.", show_alert=True)
        return
    ...
    req = db.execute("SELECT * FROM requests WHERE id=%s AND status='open'", (req_id,), fetch="one")
    if not req:
        await q.answer("⚠️ This request is no longer available.", show_alert=True)
        return
    ...
    await q.edit_message_text(
        ...
        "_Use /myapplications or 'Applied Postings' to track your applications._",
        parse_mode="Markdown"
    )
    ...
    async def notify_tutor():
        try:
            await ctx.bot.send_message(
                ...
                "_Contact your admin to arrange payment._\n\n"
                "_Reach out to the parent on WhatsApp to arrange the first lesson. Good luck!_",
                parse_mode="Markdown"
            )
        except Exception as e:
...
async def open_requests(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for r in reqs:
        ...
        line = (
            icon + "  *#" + str(r["id"]) + "* — " + r["subject"] + " | " +
            r["level"] + " | " + rate_str(r["budget"]) + "\n"
            "    Parent: " + r["parent"] + "\n"
            "    Applicants: *" + str(apps) + "*"
        )
        if apps > 0:
...
async def view_applicants(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        hdr("📊", "Applicants for Request #" + str(req_id)) + "\n\n" +
        fld("Subject", req["subject"]) + "\n" +
        fld("Level",   req["level"]) + "\n" +
        fld("Area",    req["areas"]) + "\n" +
        fld("Budget",  rate_str(req["budget"])) + "\n" +
        ...
        DIV + "\n*" + str(len(apps)) + " Applicant" + ("s" if len(apps) != 1 else "") +
    ...
    if kb_rows:
        await update.message.reply_text(
            "Select a tutor to confirm the match:", reply_markup=InlineKeyboardMarkup(kb_rows)
        )
...
async def rej_tutor_reason(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Also remove role lock so they can re-register if they fix their application
...
async def remove_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        hdr("✅", "Admin Removed") + "\n\nUser `" + str(rid) + "` removed.", parse_mode="Markdown"
    )
...
async def terms_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "▸ Placement fee of $" + str(PLACEMENT_FEE) + " per successful match\n"
...
async def back_to_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Change 4: route to existing role dashboard instead of showing role select."""
    q = update.callback_query
    await q.answer()
    uid  = q.from_user.id
    role = get_user_role(uid)
    ...
    # Brand-new user with no role yet
    kb = [[
        InlineKeyboardButton("👨‍🏫 I am a Tutor",  callback_data="role_tutor"),
        InlineKeyboardButton("👨‍👩‍👧 I am a Parent", callback_data="role_parent"),
    ]]
...
async def profile_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    reqs = db.execute(
        "SELECT name, phone, username FROM requests WHERE parent_id=%s LIMIT 1", (uid,), fetch="one"
    )
    if reqs:
        await update.message.reply_text(
            hdr("👨‍👩‍👧", "Your Parent Profile") + "\n\n" +
            fld("Name",     reqs["name"]) + "\n" +
            fld("Phone",    reqs["phone"]) + "\n" +
            fld("Telegram", "@" + reqs["username"] if reqs["username"] else "none"),
            parse_mode="Markdown"
        )
        return
...
async def myrequests_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if tutor:
        ...
        for a in apps:
            ...
            lines.append(
                "📌 *#" + str(a["request_id"]) + "* — " + a["subject"] + " | " + a["level"] + "\n"
                "   Score: " + str(a["match_score"]) + "/100 | Status: " + icon + "\n"
                "   Applied: " + a["created_at"].strftime("%d %b %Y")
            )
        await update.message.reply_text("\n\n".join(lines), parse_mode="Markdown")
        return
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            # Change 1: new P_CONFIRM state for returning parents
            P_CONFIRM:  [
                CallbackQueryHandler(p_confirm_saved,   pattern="^p_use_saved$"),
                CallbackQueryHandler(p_update_details,  pattern="^p_update_details$"),
            ],
    ...
    logger.info("CognifySG v7 is starting...")
